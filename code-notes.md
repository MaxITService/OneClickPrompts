...
- **Integration**: Site scripts call `SelectorGuard.findEditor()/findSendButton()/findStopButton()`; on repeated failures the Brain runs heuristics, shows toasts on success/error, and returns the guessed element to the caller. When both editor and send-button selectors are missing, the Guard now surfaces the editor failure toast first to match dependency order.
- **Auto-detect toggles**: If heuristics are disabled in the popup toggles, the Brain still shows a "not found; auto-detect off" toast so the user sees which element is missing even without running scans. Stop-button detection is now also covered by this logical flow.
...
- **Queue drag-and-drop (`floating-panel-ui-queue-dnd.js`)**:
  ...
  - Drag a queued item out to remove it: while the pointer is outside every queue surface (`isPointerOverAnyQueueSurface`: `resolveQueueDropTarget` hit, or within `BUTTON_DRAG_CONTAINER_MARGIN` of a visible display area) `setQueueDragRemoveIntent(true)` pulls the placeholder out so the list closes up (FLIP via `captureQueueSiblingRects`/`playQueueSiblingsFlip`) and marks the dragged item `.max-extension-queued-item--remove-intent` (dimmed + trash hint); coming back re-inserts the placeholder at its last index. `completeQueueDrag` then calls `removeFromQueue(currentIndex)` (index resolved by `queueId` at release) and `offerQueueRemovalUndo(item, index)` for the usual Undo chip. `restoreDragArtifactsAfterRender` honours `removeIntent` (no placeholder re-insert, class re-applied) so a mid-drag re-render keeps the state.
  - Unified button drag (no edit mode needed): `registerButtonDragSource(element, { buttonConfig?, queueDroppable? })` is called by `MaxExtensionButtons.createCustomSendButton` (system buttons pass `{ queueDroppable: false }`) and for separators in `generateAndAppendAllButtons`. A mouse drag starts after a 5px pointer move (plain clicks still send; button edit mode takes precedence); touch/pen pointers start by a stationary long-press (`QUEUE_LONG_PRESS_MS`, cancelled if the pointer moves more than `QUEUE_DRAG_MOVE_THRESHOLD` first, i.e. a scroll). While pending/active the source carries `.ocp-button-drag-source--pending/--active` (`touch-action: none`), gets `setPointerCapture`, and a non-passive `touchmove` listener on the source cancels the browser scroll only while that element's drag is active (so `pointercancel` never kills it); a long-press that never moves is treated as a slow tap and its click is not suppressed. The source stays in flow hidden while a fixed-position ghost follows the pointer; over the toolbar the element is reordered live via the edit-mode helpers (`moveDraggedButton`/`captureRects`/`playFlip`/`saveOrderFromDom`, which now accept an explicit container) and the new order is saved on release. Regular prompt buttons dropped onto a *visible* queue surface (inline bar/items strip, panel queue section/display) are put back in their original slot and enqueued via `addToQueue(config, { index })` (runtime `enqueueAt`); queue mode is switched on if it was off, the timer is never started/stopped, and the status chip reports the position with an Undo action (`offerQueueAddUndo(entry, text)` / `undoQueueAdd()`, `pendingQueueAddUndo`, `action: 'undo-add'` in `renderQueueStatusFromState`; Undo removes that exact entry via `removeById(queueId)` and silently clears the chip if it was already dispatched). Only the queue-full error remains a toast. Separators and system buttons are reorder-only: over a queue surface the ghost turns red (`.ocp-button-drag-ghost--blocked`) and nothing is queued. A drag never reveals a hidden/dismissed queue bar. Hovered drop zones get `.ocp-queue-drop-hover` plus an `.ocp-queue-drop-indicator` slot marker; `body.ocp-queue-button-dragging` marks visible zones with a dashed hint. Reordering only happens while the pointer is within the toolbar box (+24px); releasing further away (and not on a queue), or on a queue with a reorder-only element (red ghost), cancels and restores the original order, as does Esc. A persisted reorder (`saveOrderFromDom` resolved `true`) shows a "Buttons reordered — Undo" toast (`offerButtonReorderUndo(previousOrder, origin)` / `undoButtonReorder`, one pending at a time, a newer reorder closes the older toast): Undo restores the snapshotted `customButtons` order, saves via `MaxExtensionButtonEditMode.saveCurrentProfileConfig` and re-renders with `updateButtonsForProfileChange(origin)`. `logConCgp('[button-dnd] ...')` lines cover start, refusal (with reason), completion outcome (`reorder-saved`, `reorder-unchanged`, `queued@index`, `queue-full`, `cancelled-outside`, `blocked-on-queue`) and cancel (Esc / pointercancel). Kill switch: Advanced tab -> "Drag buttons to reorder / drop into queue" (`enableButtonDragAndDrop` in the `selectorAutoDetector` settings bag, default on, checked at pointer-down so it applies live via `selectorAutoDetectorSettingsChanged`).
  - Empty queue surfaces render a `.max-extension-queue-drop-placeholder` ("Drop a prompt button here to queue it") whose tooltip (shared via `getQueueDropZoneTooltip()`) explains drop/reorder/remove; the same tooltip is set on the inline items strip and the panel display. The inline items strip always shows it; the panel display (`#max-extension-queue-display`) stays compact (`display:none` while empty, via `applyPanelQueueDisplayVisibility`, which the queue-mode toggle/init/TOS-accept and `renderQueueDisplayInto` all share) and is revealed only during a queue-droppable button drag by `body.ocp-queue-button-dragging #max-extension-queue-display { display:flex !important }`.
...
### 3.6 ChatGPT Exporter (Markdown)
- **Purpose**: ChatGPT-only module (off by default) that adds three buttons to the inline row / floating panel: 📑 export whole chat, 🤖 export answers only, ☑️ export selected messages. Click downloads `.md`; Shift+click (first two) copies to the clipboard. No PDF.
- **Data source**: `GET /backend-api/conversation/<id>` with a bearer token from `/api/auth/session` (same-origin content-script fetch, token kept in memory ~5 min, one retry on 401). Canvas final versions from `/backend-api/conversation/<id>/textdocs`. If the API fails, falls back to the rendered messages' `innerText` and shows a warning toast.
- **Pipeline** (all under `modules/chatgpt-exporter/`, each an IIFE registering on `window.OCPChatGptExporter`):
  - `conversation-parser.js` (pure): walks the displayed branch of the `mapping` tree (leaf = `current_node` unless the DOM renders messages off that branch) and groups nodes into turns of typed blocks: `text`, `image`, `thinking` (thoughts + search steps + recap), `canvas` (create/update replay), `code`/`output` (code interpreter), `research` (new widget-state and legacy `deepresch_` Deep research).
  - `markdown-renderer.js` (pure): `\(…\)`/`\[…\]` → `$…$`/`$$…$$` outside code, genui math widgets → `$$` blocks, citation markers (U+E200…) → links or dropped, Deep research citations → per-report footnotes + References, `:::writing` blocks, safe dynamic-length code fences, thinking as blockquote, YAML front matter. Exposes `renderHeader` / `renderSection` / `assembleDocument` (their composition equals `renderDocument`) plus `previewOf(turn)` for the picker.
  - `message-picker.js`: fullscreen Shadow-DOM dialog styled like the settings popup (purple accent, light-grey canvas, white bordered list card, 4px buttons, lavender header; dark variant follows ChatGPT's theme; content centred in a 1280px column). Minimize turns it into a floating bottom-right bar (host gets `pointer-events: none`, so the chat scrolls/clicks normally) with Restore / Download / Cancel; Esc on the bar restores. Each turn is one clamped two-line row; a chevron (down = unfold, up = fold, or →/←) expands it to the full Markdown that will be exported (built lazily via `getFullText`); toolbar "Expand all" ↔ "Collapse all" toggles every visible row (batched: one reflow, only in-viewport rows animate). Folds animate via `grid-template-rows: 0fr↔1fr` + delayed `visibility`; all motion off under `prefers-reduced-motion`. All/None/Prompts/Answers/Invert act on filtered rows; Shift+click ranges, keyboard navigation, focus trap. Footer shows live selection size from `measure(ids)`: characters (code points), UTF-8 KB and, when the Token Counter module is enabled for ChatGPT, `≈ tokens (model)` using its chosen model + calibration. Resolves `{ ids, action: 'download'|'copy' }` or `null`.
  - `chatgpt-exporter.js`: settings (`getChatGptExporterSettings`, live `chatgptExporterSettingsChanged` → re-render inline + panel), API client, delivery, per-picker section cache (`createSectionCache`: each message rendered once; full text, size stats and the final selection file come from the same strings; token estimator from `getTokenApproximatorSettings` + `OCP_createTokenModelRegistry`), buttons (`createToolbarButtons()`, `getToolbarPlacement()`), `settingsReady` promise awaited by `init.js` before the first render.
- **Settings** (`modules.chatgptExporter` in StateStore, included in full backups): `{ enabled, placement: 'before'|'after', includeThinking, includeSources, icons: { full, answers, select } }`. Popup UI: `modules/popup-page-modules-chatgptExporter.js` + `#chatgptExporterModule` in `popup.html`.

### 3.7 Module Button Icons (user-editable emojis)
- Modules that inject toolbar buttons (Cross-Chat: `copy`, `paste`, `broadcast`, `shield`; ChatGPT Exporter: `full`, `answers`, `select`) store their emojis in `settings.icons`.
- `modules/module-button-icons.js` is the single source of defaults + `normalize(moduleId, raw)` (complete map; blank → default; max 2 graphemes via `Intl.Segmenter`). Classic script publishing `globalThis.OCPModuleButtonIcons`, loaded by the manifest (content scripts, before `buttons.js`), by `popup.html`, and via side-effect `import` in `service-worker-auxiliary-state-store.js`.
- Popup: small square inputs (`input.module-icon-input[data-icon-key]` in `#crossChatIconsRow` / `#chatgptExporterIconsRow`) bound by `modules/popup-page-modules-buttonIcons.js` (`OCPModuleIconInputs.bind`): select-all on focus, save on change, empty restores default. Changes reach open tabs through the existing `crossChatChanged` / `chatgptExporterSettingsChanged` broadcasts, which re-render the toolbars.
...
| ChatGPT Exporter | Export whole chat / answers only / picked messages to Markdown, keeping math, code, tables, thinking, Canvas, Deep research and citations | `modules/chatgpt-exporter/*`, `modules/popup-page-modules-chatgptExporter.js` |
