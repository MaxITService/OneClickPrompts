...
- **Queue drag-and-drop (`floating-panel-ui-queue-dnd.js`)**:
  ...
  - Prompt button → queue drops: `registerQueueDragSource(element, buttonConfig)` (called by `MaxExtensionButtons.createCustomSendButton`, skipped for system buttons via `{ queueDraggable: false }`) turns any regular prompt button into a drag source. A drag starts after a 5px pointer move (plain clicks still send; button edit mode takes precedence). While dragging, a fixed-position ghost follows the pointer, `body.ocp-queue-button-dragging` is set, and every queue surface is force-revealed (`.is-drop-ready` on inline bars, `.ocp-queue-drop-ready` on the panel queue section/display) even when dismissed, empty, or queue mode is off. Drop targets are the inline bar/items strip and the panel queue section/display; the hovered one gets `.ocp-queue-drop-hover` plus an `.ocp-queue-drop-indicator` slot marker. Dropping enables queue mode if needed (same as the "queue current editor text" button), calls `addToQueue(config, { index })` (runtime `enqueueAt`), never starts/stops the timer, and toasts the resulting position.
  - Empty queue surfaces render a `.max-extension-queue-drop-placeholder` ("Drop a prompt button here to queue it") whose tooltip (shared via `getQueueDropZoneTooltip()`) explains drop/reorder/remove; the same tooltip is set on the inline items strip and the panel display.
