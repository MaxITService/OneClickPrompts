...
- **Settings** (`modules.chatgptExporter` in StateStore, included in full backups): `{ enabled, placement: 'before'|'after', includeThinking, includeSources }`. Popup UI: `modules/popup-page-modules-chatgptExporter.js` + `#chatgptExporterModule` in `popup.html`.
