...
- **Pipeline** (all under `modules/chatgpt-exporter/`, each an IIFE registering on `window.OCPChatGptExporter`):
  ...
  - `markdown-renderer.js` (pure): `\(…\)`/`\[…\]` → `$…$`/`$$…$$` outside code, genui math widgets → `$$` blocks, citation markers (U+E200…) → links or dropped, Deep research citations → per-report footnotes + References, `:::writing` blocks, safe dynamic-length code fences, thinking as blockquote, YAML front matter. Also `previewOf(turn)` for the picker.
  - `message-picker.js`: Shadow-DOM dialog listing each turn as one clamped two-line row; All/None/Prompts/Answers/Invert act on filtered rows; Shift+click ranges, keyboard navigation, focus trap; resolves `{ ids, action: 'download'|'copy' }` or `null`.
  - `chatgpt-exporter.js`: settings (`getChatGptExporterSettings`, live `chatgptExporterSettingsChanged` → re-render inline + panel), API client, delivery, buttons (`createToolbarButtons()`, `getToolbarPlacement()`), `settingsReady` promise awaited by `init.js` before the first render.
