...
window.MaxExtensionPromptVariables = {
    ...
    dateExampleIcon: '📅',
    shineAnimationDurationMs: 900,
    shineStateLifetimeMs: 15000,
    shineState: null,
    ...
    markButtonForShine(profileIndex, kind = '') {
        this.shineState = {
            profileIndex,
            kind,
            until: Date.now() + this.shineStateLifetimeMs
        };
    },
    ...
    shouldShineButton(buttonConfig, profileIndex) {
        ...
        const isMatch = (
            (Number.isInteger(profileIndex) && profileIndex === state.profileIndex) ||
            (state.kind && buttonConfig?.__ocpSmartVariableExample === state.kind)
        );
        if (isMatch) {
            this.shineState = null;
        }
    ...
    ensureStyles() {
        ...
        style.textContent = `
            @keyframes ocp-smart-vars-button-shine {
                ...
                45% {
                    box-shadow: 0 0 0 2px rgba(37, 99, 235, 0.18), 0 0 12px rgba(37, 99, 235, 0.55);
                    transform: translateY(-1px);
                }
            }
            .ocp-smart-vars-new-button {
                border-radius: 8px !important;
                animation: ocp-smart-vars-button-shine ${this.shineAnimationDurationMs}ms ease-in-out 1;
            }
        `;
...
window.MaxExtensionButtons = {
    updateCustomButtonAutoSendTooltip: function (buttonElement) {
        if (!(buttonElement instanceof HTMLElement)) return;

        const baseTooltipText = buttonElement.dataset.ocpTooltipBase || '';
        const shortcutDescription = buttonElement.dataset.ocpTooltipShortcut || '';
        const buttonAutoSendEnabled = buttonElement.dataset.ocpButtonAutoSend === 'true';
        const autoSendDescription = buttonAutoSendEnabled
            ? ` <span class="ocp-tooltip__system-msg"><i><b>(${window.globalMaxExtensionConfig?.globalAutoSendEnabled
                ? 'Auto-sends'
                : 'Auto-sends when globally enabled'})</b></i></span>`
            : '';
        const tooltipText = `${baseTooltipText}${autoSendDescription}${shortcutDescription}`;

        buttonElement.setAttribute('title', tooltipText);
        window.OCPTooltip?.updateText?.(buttonElement, tooltipText);
    },

    refreshCustomButtonAutoSendTooltips: function (root = document) {
        root.querySelectorAll?.('[data-ocp-button-auto-send]')
            .forEach((buttonElement) => this.updateCustomButtonAutoSendTooltip(buttonElement));
    },
    ...
    createCustomSendButton: function (buttonConfig, buttonIndex, onClickHandler, overrideShortcutKey = undefined) {
        ...
        // Keep the component parts so the AutoSend footer can react immediately
        // when the per-tab global toggle changes.
        const baseTooltipText = escapeTooltipHtml(buttonConfig.tooltip || buttonConfig.text);
        customButtonElement.dataset.ocpTooltipBase = baseTooltipText;
        customButtonElement.dataset.ocpTooltipShortcut = shortcutDescription;
        customButtonElement.dataset.ocpButtonAutoSend = buttonConfig.autoSend === true ? 'true' : 'false';
        this.updateCustomButtonAutoSendTooltip(customButtonElement);
     ...
     * starts the existing queue engine with the configured initial delay unless
     * Shift is held. Shift queues the text without starting a stopped queue.
     * @param {Event} event
     * @returns {Promise<Object>}
     */
    queueCurrentEditorText: async function (event) {
        ...
        const shouldStartStoppedQueue = !event?.shiftKey;
        ...
        const isPerplexity = window?.InjectionTargetsOnWebsite?.activeSite === 'Perplexity';
        if (isPerplexity && editor.hasAttribute('data-ocp-clear-pending')) {
            this.__toast('Editor clearing is still in progress. Please wait.', 'info');
            return { status: 'ignored', reason: 'editor_clear_pending' };
        }
        ...
        // Mark the Perplexity clear before the first asynchronous persistence
        // operation so two rapid clicks cannot enqueue the same stale text.
        if (isPerplexity) {
            editor.setAttribute('data-ocp-clear-pending', 'true');
        }

        const runtimeAvailable = queue?.queueRuntime || window.MaxExtensionQueueRuntimeController;
        if (!queue || !runtimeAvailable || typeof queue.addToQueue !== 'function' || typeof queue.startQueue !== 'function') {
            if (isPerplexity) editor.removeAttribute('data-ocp-clear-pending');
            this.__toast('Queue runtime is not loaded. Reload the extension, then refresh this page.', 'error');
            logConCgp('[buttons][queue] Queue runtime unavailable. The extension manifest may not have been reloaded.');
        ...
        const queueRuntime = queue.queueRuntime;
        const persisted = typeof queueRuntime?.flushPersistence === 'function'
            && await queueRuntime.flushPersistence();
        if (!persisted) {
            queueRuntime?.removeById?.(queuedItem.queueId);
            await queueRuntime?.flushPersistence?.();
            if (isPerplexity) editor.removeAttribute('data-ocp-clear-pending');
            this.__toast('Could not save the queue item. Editor text was not cleared.', 'error');
            return { status: 'failed', reason: 'queue_save_failed' };
        }
        ...
        const clearResult = this.__clearEditor(editor);
        const cleared = clearResult && typeof clearResult.then === 'function'
            ? await clearResult
            : clearResult;
        if (!cleared) {
        ...
        if (shouldStartStoppedQueue) {
            queue.startQueue({ waitBeforeFirstSend: true });
        }
        const queueState = queue.queueRuntime?.snapshot;
        const count = queueState?.items.length ?? 0;
        const timerRunning = queueState?.isRunning === true;
        const queuedWithoutStarting = !shouldStartStoppedQueue && !timerRunning;
        this.__toast(
            queuedWithoutStarting
                ? `Queued without starting; ${count} item${count === 1 ? '' : 's'} waiting.`
                : timerRunning
                ? `Queued. Timer running; ${count} item${count === 1 ? '' : 's'} waiting.`
                : `Queued, but the timer did not start. Press Play to retry.`,
            timerRunning || queuedWithoutStarting ? 'success' : 'warning'
        );
        return { status: timerRunning ? 'queued' : 'queued_paused', count };
    },
    ...
    __showDelayFlyout: function (event) {
        ...
        const queue = window.MaxExtensionFloatingPanel;
        const currentDelay = Math.min(64000, Math.max(10, Math.round(
            typeof queue?.getQueueBaseDelayMs === 'function'
                ? queue.getQueueBaseDelayMs() / 1000
                : 60
        )));
        const currentSliderPosition = typeof queue?.queueDelayValueToSliderPosition === 'function'
            ? queue.queueDelayValueToSliderPosition(currentDelay, 10, 64000)
            : 0;
        ...
        flyout.innerHTML = `
            ...
            <div style="display: flex; flex-direction: column; gap: 4px;">
                ...
                <input type="range" id="ocp-flyout-slider" min="0" max="1000" step="1" value="${currentSliderPosition}" title="Logarithmic scale: 10–64000 seconds" style="
                    width: 100%;
                    accent-color: #10a37f;
                    cursor: pointer;
                    height: 5px;
            ...
            <div style="display: flex; align-items: center; justify-content: space-between; gap: 8px;">
                ...
                <input type="number" id="ocp-flyout-input" min="10" max="64000" value="${currentDelay}" style="
                    width: 75px;
                    background: rgba(255, 255, 255, 0.1);
        ...
        let latestDelay = currentDelay;
        let hasUnsavedDelay = false;

        const updateDelay = (val, options = {}) => {
            const { persist = false, recalculate = false } = options;
            const parsed = parseInt(val, 10);
            if (!Number.isFinite(parsed)) return;
            const clamped = typeof queue?.setQueueDelayValue === 'function'
                ? queue.setQueueDelayValue(parsed, 'sec', { persist, recalculate })
                : Math.max(10, Math.min(64000, parsed));
            if (typeof queue?.setQueueDelayValue !== 'function') {
                if (!window.globalMaxExtensionConfig) window.globalMaxExtensionConfig = {};
                window.globalMaxExtensionConfig.queueDelaySeconds = clamped;
                window.globalMaxExtensionConfig.queueDelayUnit = 'sec';
            }
            latestDelay = clamped;
            hasUnsavedDelay = !persist;

            // Sync visual labels
            valLabel.textContent = `${clamped}s`;
            const sliderPosition = typeof queue?.queueDelayValueToSliderPosition === 'function'
                ? queue.queueDelayValueToSliderPosition(clamped, 10, 64000)
                : 0;
            if (slider.value !== String(sliderPosition)) {
        ...
        slider.addEventListener('input', (e) => {
            const seconds = typeof queue?.queueDelaySliderPositionToValue === 'function'
                ? queue.queueDelaySliderPositionToValue(e.target.value, 10, 64000)
                : 10;
            updateDelay(seconds);
        });
        ...
        slider.addEventListener('change', (e) => {
            const seconds = typeof queue?.queueDelaySliderPositionToValue === 'function'
                ? queue.queueDelaySliderPositionToValue(e.target.value, 10, 64000)
                : latestDelay;
            updateDelay(seconds, { persist: true, recalculate: true });
        });
        ...
        input.addEventListener('change', (e) => {
            ...
            const clamped = Number.isFinite(val) ? Math.max(10, Math.min(64000, val)) : 60;
            updateDelay(clamped, { persist: true, recalculate: true });
        });

        const closeFlyout = () => {
            if (hasUnsavedDelay && typeof queue?.setQueueDelayValue === 'function') {
                queue.setQueueDelayValue(latestDelay, 'sec', { persist: true, recalculate: true });
                hasUnsavedDelay = false;
            }
            flyout.style.opacity = '0';
    ...
    __readEditorText: function (editor) {
        if (!editor) return '';
        if (window?.InjectionTargetsOnWebsite?.activeSite === 'Perplexity'
            && editor.hasAttribute('data-ocp-clear-pending')) {
            return '';
        }
    ...
    __clearEditor: function (editor) {
        try {
            if (window?.InjectionTargetsOnWebsite?.activeSite === 'Perplexity'
                && typeof window.clearPerplexityEditor === 'function') {
                return window.clearPerplexityEditor(editor);
            }
