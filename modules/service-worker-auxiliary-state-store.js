...
// Defaults and limits for manual queue cards (numbered 1-9).
const MANUAL_QUEUE_CARD_MIN_COUNT = 1;
const MANUAL_QUEUE_CARD_MAX_COUNT = 9;
const MANUAL_QUEUE_CARD_DEFAULT_COUNT = 6;
const MANUAL_QUEUE_CARD_DEFAULT_EMOJIS = ['1️⃣', '2️⃣', '3️⃣', '4️⃣', '5️⃣', '6️⃣', '7️⃣', '8️⃣', '9️⃣'];

function normalizeManualQueueCardCount(value, cardsLength = 0) {
  const numericValue = Number(value);
  const fallback = Number.isInteger(cardsLength) && cardsLength > 0
    ? cardsLength
    : MANUAL_QUEUE_CARD_DEFAULT_COUNT;
  const requestedCount = Number.isInteger(numericValue) ? numericValue : fallback;
  return Math.min(MANUAL_QUEUE_CARD_MAX_COUNT, Math.max(MANUAL_QUEUE_CARD_MIN_COUNT, requestedCount));
}
...
async function getValue(path) {
  ...
  if (path === KEYS.modules.manualQueueCards) {
    ...
    if (obj && typeof obj === 'object') {
      ...
      const cardCount = normalizeManualQueueCardCount(obj.cardCount, cards.length);
    ...
    return {
      cards: MANUAL_QUEUE_CARD_DEFAULT_EMOJIS
        .slice(0, MANUAL_QUEUE_CARD_DEFAULT_COUNT)
...
async function setValue(path, value) {
  ...
  if (path === KEYS.modules.manualQueueCards) {
    ...
    const cardCount = normalizeManualQueueCardCount(data.cardCount, cards.length);
    const normalizedCards = [];
    for (let i = 0; i < cardCount; i++) {
      const card = cards[i] || {};
      normalizedCards.push({
