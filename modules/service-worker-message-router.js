...
import {
    ...
    deleteQueuePersistenceSnapshot
} from './service-worker-queue-persistence.js';
...
export function handleMessage(request, sender, sendResponse) {
    switch (request.type) {
        ...
        case 'saveConfig':
            saveProfileConfig(request.profileName, request.config, {
                excludeTabId: request.suppressSenderRefresh === true ? sender?.tab?.id : null
            }).then(success => {
        ...
        // ----- Per-tab Queue Recovery Journal -----
        case 'queuePersistenceClaim':
            claimQueuePersistenceContext(request, sender)
                .then(sendResponse)
                .catch((error) => {
                    handleStorageError(error);
                    sendResponse({ success: false, error: error.message });
        ...
        case 'queuePersistenceSave':
            saveQueuePersistenceSnapshot(request, sender)
                .then(sendResponse)
                .catch((error) => {
                    handleStorageError(error);
                    sendResponse({ success: false, error: error.message });
        ...
        case 'queuePersistenceDelete':
            deleteQueuePersistenceSnapshot(request, sender)
                .then(sendResponse)
                .catch((error) => {
                    handleStorageError(error);
                    sendResponse({ success: false, error: error.message });
