...
const isOcpAutoDetectUiElement = (element) => {
    if (!element || element.nodeType !== Node.ELEMENT_NODE) return false;
    if (window.OneClickPromptsSelectorAutoDetector?.isOcpUiElement?.(element)) return true;
    try {
        const testId = (element.getAttribute?.('data-testid') || '').toLowerCase();
        return testId.startsWith('custom-send-button') || !!element.closest?.(
...
const isVisibleAutoDetectCandidate = (element) => {
    if (!element || !element.isConnected || isOcpAutoDetectUiElement(element)) return false;
    try {
...
window.OneClickPromptsSelectorAutoDetectorBase = {
    ...
    detectEditor: async function () {
        ...
        const visibleCandidates = [...new Set(candidates)].filter(isVisibleAutoDetectCandidate);
    ...
    detectSendButton: async function () {
        ...
        // 2. Filter for visibility and size
        const visibleCandidates = candidates.filter(isVisibleAutoDetectCandidate);
