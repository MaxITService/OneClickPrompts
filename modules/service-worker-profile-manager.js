...
export function normalizeProfileConfig(profile, profileName) {
    ...
    if (typeof profile.disableBrowserConsoleLogs === 'undefined') {
        profile.disableBrowserConsoleLogs = false;
        logConfigurationRelatedStuff(`Initialized missing 'disableBrowserConsoleLogs' for profile: ${profileName}`);
    }
...
export async function saveProfileConfig(profileName, config, options = {}) {
    ...
    try {
        ...
        if (configChanged && wasActiveProfile) {
            ...
            await broadcastProfileChange(profileName, config, excludeTabId, 'inline');
            await broadcastProfileChange(profileName, config, excludeTabId, 'panel');
        }
...
export async function deleteProfile(profileName) {
    ...
    try {
        ...
        if (result.currentProfile === profileName) {
            const defaultProfile = await switchProfile('Default');
            if (!defaultProfile) {
                logConfigurationRelatedStuff(`Cannot delete active profile ${profileName}: failed to switch to Default`);
                return false;
            }
        }
