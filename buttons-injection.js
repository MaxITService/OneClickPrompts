...
function buttonBoxCheckingAndInjection(enableResiliency = true, activeWebsite, options = {}) {
    ...
    if (modificationsExist && !enableResiliency) {
    ...
    const handleContainerSearchFailure = (failedSelectors, reason = 'container_not_found') => {
        ...
        const containerHeuristicsEnabled = detector?.settings?.enableContainerHeuristics === true;
        const autoFloatingFallbackEnabled = detector?.settings?.autoFallbackToFloatingPanel !== false;

        // No visible composer means this is probably Settings, preview, or another temporary
        // non-chat surface. Let the detector enter quiet cooldown instead of opening a panel.
        if (detector && (detector.hasBlockingNonChatSurface?.() === true || !detector.findVisibleNativeEditor?.())) {
    ...
    // If this tab was healthy moments ago and the whole composer disappeared, the user most
    // likely opened Settings/preview. Do not poll broken selectors or summon recovery UI; the
    // detector's quiet observer will retry as soon as a real composer comes back.
    if (window.OneClickPromptsSelectorAutoDetector?.shouldDeferPassiveContainerSearch?.() === true) {
        logConCgp('[button-injection] Deferring inline search during a temporary non-chat surface.');
        window.__OCP_inlineSearchController = null;
        reportFailureToAutoDetector([], 'recent_surface_without_composer');
        return;
    }
    ...
    const handleTargetDiv = (targetDiv) => {
        ...
        if (!targetFound) {
            ...
            window.OneClickPromptsSelectorAutoDetector?.reportRecovery?.('container');
