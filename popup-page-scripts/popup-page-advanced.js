...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadHeuristicsSettings() {
        ...
        try {
            ...
            if (!response?.settings) {
                throw new Error(response?.error || 'Auto-Detector settings were not returned.');
            }
            heuristicsSettings = {
                ...heuristicsDefaults,
                ...response.settings,
    ...
    async function saveHeuristicsSettings(settingsSnapshot = heuristicsSettings) {
        try {
            ...
            if (response?.success !== true) {
                throw new Error(response?.error || 'The service worker rejected the settings update.');
            }
            return true;
        } catch (error) {
    ...
    const heuristicsControls = [
        editorHeuristicsToggle,
        sendButtonHeuristicsToggle,
        stopButtonHeuristicsToggle,
    ...
    async function updateHeuristicsSetting(key, checkbox) {
        const previousValue = heuristicsSettings[key];
        const nextValue = checkbox.checked;
        heuristicsSettings = { ...heuristicsSettings, [key]: nextValue };
        heuristicsControls.forEach(control => { control.disabled = true; });
        try {
            const saved = await saveHeuristicsSettings({ ...heuristicsSettings });
            if (saved) return;
            heuristicsSettings = { ...heuristicsSettings, [key]: previousValue };
            checkbox.checked = !!previousValue;
            showToast('Auto-Detector setting was not saved.', 'error');
        } finally {
            heuristicsControls.forEach(control => { control.disabled = false; });
        }
    }
        ...
        try {
            ...
            for (const option of options) {
                ...
                try {
                    ...
                    if (response?.success !== true) {
                        throw new Error(response?.error || 'The service worker rejected the reset.');
                    }
                } catch (err) {
    ...
    if (editorHeuristicsToggle) {
        editorHeuristicsToggle.addEventListener('change', () => {
            void updateHeuristicsSetting('enableEditorHeuristics', editorHeuristicsToggle);
        });
    }
    if (sendButtonHeuristicsToggle) {
        sendButtonHeuristicsToggle.addEventListener('change', () => {
            void updateHeuristicsSetting('enableSendButtonHeuristics', sendButtonHeuristicsToggle);
        });
    }
    if (stopButtonHeuristicsToggle) {
        stopButtonHeuristicsToggle.addEventListener('change', () => {
            void updateHeuristicsSetting('enableStopButtonHeuristics', stopButtonHeuristicsToggle);
        });
    }
    if (containerHeuristicsToggle) {
        containerHeuristicsToggle.addEventListener('change', () => {
            void updateHeuristicsSetting('enableContainerHeuristics', containerHeuristicsToggle);
        });
    }
    ...
    if (autoFloatingFallbackCheckbox) {
        autoFloatingFallbackCheckbox.addEventListener('change', () => {
            void updateHeuristicsSetting('autoFallbackToFloatingPanel', autoFloatingFallbackCheckbox);
        });
    }
